...
// PID computation unit must be unique. It was declared in quad_manipulator.cpp, 
// hence externed here to the use exact same unit.
...
void Wrapper::QuadController::quadPoseCallback(const std::string& robot_frame,const geometry_msgs::PoseStamped::ConstPtr& msg) 
{
	...
	if(correspondingQuad->shouldWait() == false)
	{
		...
		if(!slowing && operating){
		...
		else if (slowing && operating){ // In this context, slowing is arrived when there is a risk of collision.
			...
			vel_msg.linear.z = z * slowing_factor;
